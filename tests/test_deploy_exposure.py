...
def test_toxicant_dose_monotone_in_time_and_ppe():
    ...
    smoky = FireScenario("c", "grass", 4.0, pm25=100.0, temp_c=30.0, wind_speed=8.0, wind_dir=70.0, duration_min=120.0)
    assert toxicant_dose(smoky, 20, "ba") > toxicant_dose(SCN, 20, "ba")  # more smoke -> more dose
...
def test_heat_load_rises_with_temp_and_exertion():
    ...
    assert heat_load(hot, 20, "ba") > heat_load(SCN, 20, "ba")  # hotter -> more load
    assert heat_load(SCN, 20, "ba") > heat_load(SCN, 20, "command")  # exertion
